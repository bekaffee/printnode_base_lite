...
class PrintNodeAccount(models.Model):
    ...
    def update_computer_printers_status(self):
        """
        Update all computer's printer's status
        """
        computers_state = {}
        for computer in self._send_printnode_request('computers'):
            computers_state[computer["id"]] = computer["state"]

        self.update_status_custom(self.computer_ids, computers_state, "disconnected")

        printers_state = {}
        printer_ids = set()
        for computer in self.computer_ids:
            get_printers_url = f"computers/{computer.printnode_id}/printers"

            for printer in self._send_printnode_request(get_printers_url):
                printers_state[printer["id"]] = printer["state"]

            for printer in computer.printer_ids:
                printer_ids.add(printer)

        self.update_status_custom(printer_ids, printers_state, "offline")
        ...
        return {
            ...
            'params': {
                ...
                'message': _('All Computer and Printer statuses has been refreshed'),
                'sticky': False,
    ...
    def update_status_custom(self, records, state_map, default_status):
        for record in records:
            if record.printnode_id in state_map:
                if record.status != state_map[record.printnode_id]:
                    record.write({'status': state_map[record.printnode_id]})
                state_map.pop(record.printnode_id)
            elif record.status != default_status:
                record.write({'status': default_status})

    def cron_update_computer_printers_status(self):
        """
        Update compuer and printers statuses daily
        """
        for rec in self.env['printnode.account'].search([]):
            rec.update_computer_printers_status()
