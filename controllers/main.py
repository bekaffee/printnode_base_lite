...
class ReportControllerProxy(ReportController):
    ...
    def _check_direct_print(self, data, context):
        ...
        if not user.has_group(SECURITY_GROUP) \
            ...
            print_data["reject_reason"] = "Direct print is not enabled for users!"
            return print_data

        # STEP 2: If we are requesting not PDF or Text file, than also return
        # to standard Odoo behavior.
        if report_type not in SUPPORTED_REPORT_TYPES:
            print_data["reject_reason"] = "You can only request PDF of Text file!"
            return print_data
        
        # STEP 3: check for the printer_id, and if it's offline then return error msg
        if printer_id:
            printer_id = request.env['printnode.printer'].browse(printer_id)

            # if printer_id.status == 'offline':
            printer_id.account_id.update_computer_printers_status()

            if printer_id.status == 'offline':

                print_data["reject_reason"] = "Printer's state is offline!"
                return print_data
        ...
        # print("AXAXA", printer_id)
        # print("OLOLOLOL", print_data)
        ...
        if not printer_id:
            ...
            print_data["reject_reason"] = "Select at least printer for user or company!"
            return print_data
        
        # print("AXAXA 2", printer_id)
        # print("OLOLOLOL 2", print_data)
        ...
        # print("AXAXA 3", printer_id)
        # print("OLOLOLOL 3", print_data)
    ...
    def report_check(self, data, context=None):
        ...
        if print_data["can_print"]:
            return json.dumps({'can_print': True})
        return json.dumps({'can_print': False, 'reject_reason': print_data["reject_reason"]})
    ...
    def report_print(self, data, context=None):
        ...
        if not print_data['can_print']:
            return json.dumps({
                ...
                'message': _(print_data["reject_reason"]),
                'success': False,
                'notify': True,
            })
